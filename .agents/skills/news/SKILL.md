...
- **Older News Retention & Media**: Never delete or replace existing post files in `_posts/`. The newest post becomes the primary news feature (`site.posts | first`), while older posts automatically remain accessible under the collapsible "Ältere News" section (`offset:1`). Media assets associated with older posts MUST be displayed alongside their entries without `autoplay` (using native `controls` and `preload="metadata"`).
